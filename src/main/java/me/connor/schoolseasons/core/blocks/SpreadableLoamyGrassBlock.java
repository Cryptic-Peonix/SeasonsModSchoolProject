...
public class SpreadableLoamyGrassBlock extends SnowyDirtBlock {
    protected SpreadableLoamyGrassBlock(Properties builder) {
        super(builder);
    }
    ...
    public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
        if (!func_220257_b(state, worldIn, pos)) {
            ...
            worldIn.setBlockState(pos, BlockInit.LOAMY_DIRT.getDefaultState());
        } else {
            if (worldIn.getLight(pos.up()) >= 9) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    if (worldIn.getBlockState(blockpos).getBlock() == BlockInit.LOAMY_DIRT && func_220256_c(blockstate, worldIn, blockpos)) {
